...
class Trader:
    ...
    def _trade_product(
        ...
        for ask_price, ask_volume in asks:
            ...
            elif ask_price <= fair_value and working_position < config.target_position:
                quantity = min(buy_headroom, available, config.target_position - working_position)
                if quantity > 0:
                    orders.append(Order(product, int(ask_price), int(quantity)))
                    buy_headroom -= quantity
                    working_position += quantity
        ...
        for bid_price, bid_volume in bids:
            ...
            if bid_price >= fair_value + config.take_sell_edge:
                ...
                working_position -= quantity
            elif bid_price >= fair_value and working_position > config.target_position:
                quantity = min(sell_headroom, available, working_position - config.target_position)
                if quantity > 0:
                    orders.append(Order(product, int(bid_price), -int(quantity)))
                    sell_headroom -= quantity
                    working_position -= quantity

        reservation_price = fair_value - config.inventory_penalty * (working_position - config.target_position)
        ...
        inventory_gap = config.target_position - working_position
    ...
    def _quote_prices(
        ...
        bid_ceiling = math.floor(fair_value - config.quote_clear_edge)
        ask_floor = math.ceil(fair_value + config.quote_clear_edge)
